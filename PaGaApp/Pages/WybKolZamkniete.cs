...
{   
    public partial class WybKolZamkniete : Form
    {
        ...
        public void wyplenienie()
        {
            ...
            {
                if (Properties.Settings.Default.Col1Done.Count > 0)
                {
                    foreach (var item in Properties.Settings.Default.Col1Done)
                    {
                ...
                {
                    if (Properties.Settings.Default.Col2Done.Count == 0)
                    {
                        foreach (var item in Properties.Settings.Default.ColZlecenia)
                        {
                            listadoWyswietlenia.Add(item);
                            VisCol.Items.Add(listadoWyswietlenia[listadoWyswietlenia.IndexOf(item)]);
                        }
                ...
                if (Properties.Settings.Default.Col2Done.Count > 0)
                {
                    foreach (var item in Properties.Settings.Default.Col2Done)
                    {
        ...
        private void WybKolOtwarte_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (Properties.Settings.Default.Col1Done.Count>0)
                Properties.Settings.Default.Col1Done.Clear();
            if (Properties.Settings.Default.Col2Done.Count>0)
                Properties.Settings.Default.Col2Done.Clear();
            Properties.Settings.Default.Col1Done = col1;
            Properties.Settings.Default.Col2Done = col2;
